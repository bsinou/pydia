 ...
 */
public class CellsSigner extends AWS4Signer {
    private static final String CONTENT_SHA_256 = "AWS4-HMAC-SHA256";
    ...
    protected void processRequestPayload(Request<?> request, HeaderSigningResult headerSigningResult) {
        if (useChunkEncoding(request)) {
            ...
            AwsChunkedEncodingInputStream chunkEncodedStream = new AwsChunkedEncodingInputStream(
                    payloadStream, kSigning, dateTime, keyPath, signature, this);
            request.setContent(chunkEncodedStream);
        }
    }
