...
//    }
    ...
    protected val servers: Map<String, Server>
        protected get() = serverStore.getAll()
    ...
    override fun registerAccountCredentials(
        ...
        val accountID = accountID(credentials.username, serverURL)
        var server = serverStore[serverURL.id]
        if (server == null) {
            server = registerServer(serverURL)
        }
        val transport: Transport= CellsTransport(credentialService, credentials.username, server, encoder)
        ...
        if (credentials is PasswordCredentials) {
            ...
            credentialService.putPassword(accountID, pc.password)
        ...
        transportStore.put(accountID(credentials.username, server), transport)
        return accountID
    }
